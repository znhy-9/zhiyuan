<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'" label="礼品编号" prop="lipinbianhao" >
					<el-input v-model="ruleForm.lipinbianhao" placeholder="礼品编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.lipinbianhao" label="礼品编号" prop="lipinbianhao" >
					<el-input v-model="ruleForm.lipinbianhao" placeholder="礼品编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-if="type!='info'"  label="礼品数量" prop="lipinshuliang" >
					<el-input v-model.number="ruleForm.lipinshuliang" placeholder="礼品数量" clearable  :readonly="ro.lipinshuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="礼品数量" prop="lipinshuliang" >
					<el-input v-model="ruleForm.lipinshuliang" placeholder="礼品数量" readonly></el-input>
				</el-form-item>
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						if(o=='lipinbianhao'){
							this.ruleForm.lipinbianhao = obj[o];
							this.ro.lipinbianhao = true;
							continue;
						}
						if(o=='lipinmingcheng'){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jianjie = this.ruleForm.jianjie.replace(reg,'../../../springbootj09hp490/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.lipinbianhao) {
						this.ruleForm.lipinbianhao = String(this.ruleForm.lipinbianhao)
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jifenlipin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jifenlipinCrossAddOrUpdateFlag = false;
											this.parent.search();
